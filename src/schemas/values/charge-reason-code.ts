...
export type ChargeReason = Brand.Branded<string, 'ChargeReason'>;
...
export function chargeReasonCodeSchema(error?: string) {
  ...
  return z.string(error).check(z.refine(val => chargeReasonCodesKeys.includes(val as (typeof chargeReasonCodesKeys)[number]), error));
}
