...
import { validateCenEn16931BrCo10 } from './cen-en16931-br-co-10';

describe('CEN-EN16931-BR-CO-10', () => {
  it('passes on the base example', async () => {
    const document = await decodeBaseExample();
    expect(validateCenEn16931BrCo10(document).passed).toEqual(true);
  });

  it('fails when the rule is violated', async () => {
    const document = (await decodeBaseExample()) as any;
    document.legalMonetaryTotal.lineExtensionAmount = { ...document.legalMonetaryTotal.lineExtensionAmount, value: 1 };
    expect(validateCenEn16931BrCo10(document).passed).toEqual(false);
  });
});
