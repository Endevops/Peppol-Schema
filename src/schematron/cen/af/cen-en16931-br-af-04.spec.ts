/**
 * @description Unit tests for CEN-EN16931-BR-AF-04.
 */
import { describe, expect, it } from 'vitest';

import { decodeBaseExample } from '#/test/test-utils';

import { validateCenEn16931BrAf04 } from './cen-en16931-br-af-04';

describe('CEN-EN16931-BR-AF-04', () => {
  it('passes on the base example', async () => {
    const document = await decodeBaseExample();
    expect(validateCenEn16931BrAf04(document).passed).toEqual(true);
  });
  ...
  it('fails when the rule is violated', async () => {
    ...
    expect(validateCenEn16931BrAf04(document).passed).toEqual(false);
  });
});
