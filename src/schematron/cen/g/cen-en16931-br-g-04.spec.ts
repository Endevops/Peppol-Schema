/**
 * @description Unit tests for CEN-EN16931-BR-G-04.
 */
import { describe, expect, it } from 'vitest';

import { decodeBaseExample } from '#/test/test-utils';

import { validateCenEn16931BrG04 } from './cen-en16931-br-g-04';

describe('CEN-EN16931-BR-G-04', () => {
  it('passes on the base example', async () => {
    const document = await decodeBaseExample();
    expect(validateCenEn16931BrG04(document).passed).toEqual(true);
  });
  ...
  it('fails when the rule is violated', async () => {
    ...
    expect(validateCenEn16931BrG04(document).passed).toEqual(false);
  });
});
