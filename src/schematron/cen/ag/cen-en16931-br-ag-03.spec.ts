/**
 * @description Unit tests for CEN-EN16931-BR-AG-03.
 */
import { describe, expect, it } from 'vitest';

import { decodeBaseExample } from '#/test/test-utils';

import { validateCenEn16931BrAg03 } from './cen-en16931-br-ag-03';

describe('CEN-EN16931-BR-AG-03', () => {
  it('passes on the base example', async () => {
    const document = await decodeBaseExample();
    expect(validateCenEn16931BrAg03(document).passed).toEqual(true);
  });
  ...
  it('fails when the rule is violated', async () => {
    ...
    expect(validateCenEn16931BrAg03(document).passed).toEqual(false);
  });
});
