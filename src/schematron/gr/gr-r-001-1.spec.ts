...
import { validateGrR001_1 } from './gr-r-001-1.ts';
...
describe('GR-R-001-1 (invoice id has 6 segments)', () => {
  it.effect(
    ...
    Effect.fn(function* () {
      ...
      yield* validateGrR001_1(document);
    })
  );

  it.effect(
    'fails when the invoice id does not have 6 segments',
    Effect.fn(function* () {
      ...
      const result = yield* validateGrR001_1(altered).pipe(Effect.result);
      assert(Result.isFailure(result));
    })
  );

  it.effect(
    'passes when the invoice id has 6 segments',
    Effect.fn(function* () {
      const document = yield* Effect.promise(async () => asGreek(await decodeBaseExample()));
      const altered = { ...document, id: VALID_GREEK_ID } as unknown as PeppolDocument;
      yield* validateGrR001_1(altered);
    })
  );
});
