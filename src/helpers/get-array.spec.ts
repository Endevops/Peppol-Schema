...
describe('getArray', () => {
  it('returns [] when the node is undefined', () => {
    expect(getArray(undefined, 'cac:Thing')).toEqual([]);
  });

  it('returns [] when the node is null', () => {
    expect(getArray(null, 'cac:Thing')).toEqual([]);
  });

  it('returns [] when the path is missing', () => {
    expect(getArray({}, 'cac:Thing')).toEqual([]);
  });

  it('returns [] when an intermediate path segment is falsy', () => {
    expect(getArray({ 'cac:Thing': null }, 'cac:Thing', 'cbc:ID')).toEqual([]);
    expect(getArray({ 'cac:Thing': undefined }, 'cac:Thing', 'cbc:ID')).toEqual([]);
  });

  it('returns [value] for a single object node', () => {
    expect(getArray({ 'cac:Thing': { 'cbc:ID': '1' } }, 'cac:Thing')).toEqual([{ 'cbc:ID': '1' }]);
  });

  it('returns the value as-is for an array node', () => {
    const items = [{ 'cbc:ID': '1' }, { 'cbc:ID': '2' }];
    expect(getArray({ 'cac:Thing': items }, 'cac:Thing')).toEqual(items);
  });

  it('flattens nested arrays through path traversal', () => {
    const node = { 'cac:Thing': [{ 'cbc:ID': '1' }, { 'cbc:ID': '2' }] };
    expect(getArray(node, 'cac:Thing', 'cbc:ID')).toEqual(['1', '2']);
  });

  it('flattens and drops items missing the final key', () => {
    const node = { 'cac:Thing': [{ 'cbc:ID': '1' }, { notAnId: true }, { undefined: true }, {}] };
    expect(getArray(node, 'cac:Thing', 'cbc:ID')).toEqual(['1']);
  });

  it('returns [] when the final value is falsy', () => {
    expect(getArray({ 'cac:Thing': { 'cbc:ID': null } }, 'cac:Thing', 'cbc:ID')).toEqual([]);
  });
});
