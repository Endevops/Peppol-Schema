import { Schema } from 'effect';

import { identifierSchema } from './fields/identifier-schema';

export const messageLevelResponsePartySchema = Schema.Struct({
  /**
   * @description Identifies the sender party's electronic address.
   *
   * @example
   *   7300010000001;
   *
   * @summary Sender/Receiver party's electronic address
   *
   * @name `cbc:EndpointID (+ @schemeID)`
   */
  endpointId: Schema.optionalKey(identifierSchema()),
});

export type PeppolMessageLevelResponseParty = typeof messageLevelResponsePartySchema.Type;
