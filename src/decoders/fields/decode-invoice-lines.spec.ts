...
describe('decodeInvoiceLines', () => {
  it('returns undefined when the invoice line path is missing', () => {
    const result = decodeInvoiceLines({}, 'cac:InvoiceLine');

    expect(result).toBeUndefined();
  });

  it('decodes a present, non-empty array of invoice lines', () => {
    const result = decodeInvoiceLines({ 'cac:InvoiceLine': [{ 'cbc:InvoicedQuantity': '4', 'cbc:ID': '1' }] }, 'cac:InvoiceLine');
  ...
  it('wraps a single (non-array) invoice line node into an array', () => {
    const result = decodeInvoiceLines({ 'cac:InvoiceLine': { 'cbc:InvoicedQuantity': '5' } }, 'cac:InvoiceLine');
