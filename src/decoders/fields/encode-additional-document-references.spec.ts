...
describe('encodeAdditionalDocumentReferences', () => {
  it('returns undefined when the references array is undefined', () => {
    ...
    expect(encodeAdditionalDocumentReferences(undefined)).toBeUndefined();
  });

  it('returns a ref with no attachment when attachment is absent', () => {
    // ❌ Negative: attachment undefined → no cac:Attachment.
    expect(encodeAdditionalDocumentReferences([{ id: { id: 'R1' } }])?.[0]?.['cac:Attachment']).toBeUndefined();
  });
  ...
  it('encodes an attachment with only an embedded binary object', () => {
    ...
    expect(
      encodeAdditionalDocumentReferences([{ id: { id: 'R1' }, attachment: { embeddedDocumentBinaryObject: embedded } }])?.[0]?.['cac:Attachment']
    ).toEqual({
  ...
  it('encodes an attachment with only an external reference', () => {
    ...
    expect(
      encodeAdditionalDocumentReferences([{ id: { id: 'R1' }, attachment: { externalReference: { uri: 'http://example.com/doc' } } }])?.[0]?.[
        'cac:Attachment'
      ]
