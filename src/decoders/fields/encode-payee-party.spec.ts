...
describe('encodePayeeParty', () => {
  it('returns undefined when payee party is missing', () => {
    ...
    expect(Effect.runSync(encodePayeeParty(undefined))).toBeUndefined();
  });

  it('encodes a payee party with only a name', () => {
    // ❌ Negative: partyLegalEntity and partyIdentification are absent.
    expect(Effect.runSync(encodePayeeParty({ partyName: { name: 'Payee Ltd' } }))).toEqual({
  ...
  it('omits company id when party legal entity has no company id', () => {
    ...
    expect(Effect.runSync(encodePayeeParty({ partyName: { name: 'Payee Ltd' }, partyLegalEntity: {} }))?.['cac:PartyLegalEntity']).toEqual({
  ...
  it('omits identification id when party identification has no id', () => {
    ...
    expect(Effect.runSync(encodePayeeParty({ partyName: { name: 'Payee Ltd' }, partyIdentification: {} }))?.['cac:PartyIdentification']).toEqual({
      'cbc:ID': undefined,
    });
  });
});
