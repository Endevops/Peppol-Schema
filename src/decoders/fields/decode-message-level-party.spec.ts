...
describe('decodeMessageLevelParty', () => {
  it('returns undefined when the party path is missing', () => {
    const result = Effect.runSync(decodeMessageLevelParty({}, 'cac:AccountingSupplierParty'));
  ...
  it('decodes a present but empty party node to an undefined endpoint', () => {
    const result = Effect.runSync(decodeMessageLevelParty({ 'cac:AccountingSupplierParty': {} }, 'cac:AccountingSupplierParty'));
