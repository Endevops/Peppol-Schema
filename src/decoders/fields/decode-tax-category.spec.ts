...
describe('decodeTaxCategory', () => {
  it('returns undefined when the tax category path is missing', () => {
    const result = Effect.runSync(decodeTaxCategory({}, 'cac:TaxCategory'));
  ...
  it('decodes a present tax category node', () => {
    const result = Effect.runSync(
      decodeTaxCategory({ 'cac:TaxCategory': { 'cac:TaxScheme': { 'cbc:ID': 'VAT' }, 'cbc:ID': 'S', 'cbc:Percent': '25' } }, 'cac:TaxCategory')
    );
  ...
  it('decodes a present but empty node to undefined fields', () => {
    const result = Effect.runSync(decodeTaxCategory({ 'cac:TaxCategory': {} }, 'cac:TaxCategory'));
