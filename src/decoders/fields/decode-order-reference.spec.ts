...
describe('decodeOrderReference', () => {
  it('returns undefined when the order reference path is missing', () => {
    const result = Effect.runSync(decodeOrderReference({}, 'cac:OrderReference'));
  ...
  it('decodes a present order reference node', () => {
    const result = Effect.runSync(
      decodeOrderReference({ 'cac:OrderReference': { 'cbc:ID': 'ORD-1', 'cbc:SalesOrderID': 'SO-1' } }, 'cac:OrderReference')
    );
  ...
  it('decodes a present but empty order reference node to undefined ids', () => {
    const result = Effect.runSync(decodeOrderReference({ 'cac:OrderReference': {} }, 'cac:OrderReference'));
