...
const decodeCreditNoteFromUnknown = SchemaParser.decodeUnknownEffect(PeppolCreditNote);
export const decodeCreditNote = Effect.fn(function* (value: XmlNode) {
  const root = value || {};
  const doc: XmlNode = (yield* getProp(root, 'ubl:CreditNote')) ?? root; // accept either the whole JSON or just the CreditNote node

  return yield* decodeCreditNoteFromUnknown({
