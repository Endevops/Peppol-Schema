...
describe('decodeMessageLevelResponse', () => {
  it('handles an undefined value', () => {
    const out = Effect.runSync(decodeMessageLevelResponse(undefined));
    expect(out.id).toBeUndefined();
    expect(out.documentResponse).toBeUndefined();
  });

  it('returns undefined for each missing child of a present DocumentResponse', () => {
    const out = Effect.runSync(decodeMessageLevelResponse({ 'ubl:ApplicationResponse': { 'cac:DocumentResponse': {} } } as never));
  ...
  it('returns undefined for a LineResponse entry without a Response', () => {
    const out = Effect.runSync(
      decodeMessageLevelResponse({ 'ubl:ApplicationResponse': { 'cac:DocumentResponse': { 'cac:LineResponse': [{}] } } } as never)
    );
