...
import { decodeInvoice } from './decode-invoice.ts';

describe('decodeInvoice()', () => {
  it.effect(
    'accepts a bare root without the ubl:Invoice wrapper',
    Effect.fn(function* () {
      const out = yield* decodeInvoice({
        ...
        Invoice: {
          ...
          'cac:BillingReference': undefined,
          ...
          'cac:InvoiceLine': [
            {
              ...
              'cbc:CreditedQuantity': undefined,
              'cbc:ID': '1',
              'cbc:InvoicedQuantity': { '#text': 7, '@unitCode': 'DAY' },
            ...
            },
            {
              ...
              'cbc:CreditedQuantity': undefined,
              'cbc:ID': '2',
              'cbc:InvoicedQuantity': { '#text': -3, '@unitCode': 'DAY' },
          ...
          'cac:ProjectReference': undefined,
      ...
      expect(out).toMatchSnapshot('invoice');
    })
  );

  it.effect(
    'decodes a wrapped ubl:Invoice root',
    Effect.fn(function* () {
      const out = yield* decodeInvoice({
        '?xml': { '@encoding': 'UTF-8', '@version': '1.0' },
        'ubl:Invoice': {
      ...
      expect(out).toMatchSnapshot('with ubl ns');
    })
  );
});
